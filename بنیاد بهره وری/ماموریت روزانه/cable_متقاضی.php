...
class calssName
{
    ...
    public function execute(ezcWorkflowExecution $execution, ezcWorkflowNodeAction $caller = null)
    {
        ...
        $execution->workflow->myForm->setFieldValueByName('Field_15', $PID);
        ...
        $GID = $recs;
        $execution->workflow->myForm->setFieldValueByName('Field_20', $GID);
        ...
        $newTitle = 'ماموریت روزانه (سازمان) '.$Name;
        $sql_title = "UPDATE oa_document SET DocDesc='".$newTitle."',Subject='".$newTitle."' WHERE RowID=".$docID."  limit 1";
        $db->execute($sql_title);

        $execution->workflow->myForm->setFieldValueByName('Field_19','درج نشده است');
        $execution->workflow->myForm->setFieldValueByName('Field_18','1');
    }
}
?>
