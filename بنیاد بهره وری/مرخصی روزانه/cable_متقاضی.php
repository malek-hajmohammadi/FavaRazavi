...
class calssName
{
    ...
    public function execute(ezcWorkflowExecution $execution, ezcWorkflowNodeAction $caller = null)
    {
        ...
        $execution->workflow->myForm->setFieldValueByName('Field_1', $PID);
        ...
        $GID = $recs;
        $execution->workflow->myForm->setFieldValueByName('Field_15', $GID);
        ...
        $newTitle = 'مرخصی روزانه (سازمان) ' . $Name;
        $sql_title = "UPDATE oa_document SET DocDesc='" . $newTitle . "',Subject='" . $newTitle . "' WHERE RowID=" . $docID . "  limit 1";
        $db->execute($sql_title);

        $execution->workflow->myForm->setFieldValueByName('Field_14', 'درج نشده است');
        $execution->workflow->myForm->setFieldValueByName('Field_11', '1');
    }
}
