...
class MainAjax
{
    ...
    private function getInput(){
        ...
        $this->persoalId = Request::getInstance()->varCleanFromInput('PID');
    ...
    private function buildCalendarTable(){
        ...
        $calendarTable="";
        $calendarTable.="<table style='border: #ccc 2px solid'>";
    ...
    private function buildCreateFormBoxTd($row){
        ...
        MainAjax::MORKHASI_SAATI_FORM_ID;
        $db = WFPDOAdapter::getInstance();
        $wfidMorkhasiS = $db->executeScalar("SELECT max(`workflow_id`) FROM `wf_workflow` WHERE `workflow_enable` = 1 AND `workflow_formtypeid` ={${MainAjax::MORKHASI_SAATI_FORM_ID}}");//962
        $wfidMorkhasiR = $db->executeScalar("SELECT max(`workflow_id`) FROM `wf_workflow` WHERE `workflow_enable` = 1 AND `workflow_formtypeid` = {${MainAjax::MORKHASI_ROOZANEH_FORM_ID}}");//931
        $wfidMamoriatS = $db->executeScalar("SELECT max(`workflow_id`) FROM `wf_workflow` WHERE `workflow_enable` = 1 AND `workflow_formtypeid` = {${MainAjax::MAMORIAT_SAATI_FORM_ID}}");//963
        $wfidMamoriatR = $db->executeScalar("SELECT max(`workflow_id`) FROM `wf_workflow` WHERE `workflow_enable` = 1 AND `workflow_formtypeid` = {${MainAjax::MAMORIAT_ROOZANEH_FORM_ID}}");//960
        ...
        $btnsBox = '<div class="f-button-1" onmousedown="FormOnly.allFieldsContianer[3].MamoriatR('.$wfidMamoriatR.',\''.$date.'\')">ماموريت<br>روزانه</div>';
        $btnsBox .= '<div class="f-button-1" onmousedown="FormOnly.allFieldsContianer[3].MorkhasiR('.$wfidMorkhasiR.',\''.$date.'\')">مرخصي<br>روزانه</div>';
        ...
        $btnsBox .= '<div class="f-button-1" onmousedown="FormOnly.allFieldsContianer[3].MorkhasiS('.$wfidMorkhasiS.',\''.$date.'\')">مرخصی<br>ساعتی</div>';


        $link = "
        <td><a class='f-button-float-1' href='#Modal$this->trNumber'>ايجاد فرم</a>
