...
class calssName
{
    public function execute($self)
    {
        ...
        $strHTML .= '<tr><td>شماره پرسنلي</td><td width="120"> نام و نام خانوادگي</td><td width="120">سمت</td><td style="direction:ltr">پيوست</td></tr>';
        //$pathst = "/mnt/ahkam/";
            ...
            if ($handle = opendir($pathst)) {
                while (false !== ($file = readdir($handle))) {
                    if ($file != "." && $file != ".." && $file[0] != ".") {
                        ...
                        if (strlen($personID) == 3) $personID = '0' . $personID;
                        ...
                        $RID = $person['RID'];
                        $res[] = $person;
                        $strHTML .= '<tr><td>' . $personID . '</td><td>' . $person["fname"] . '  ' . $person["lname"] . '</td><td>' . $person["Name"] . '</td><td>' . $real_name . '</td></tr>';
                    }
                }
                $strHTML .= '</table></div>';
            }
        }
        return array("res" => $strHTML);
    }
}
