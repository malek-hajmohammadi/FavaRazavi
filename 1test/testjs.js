this.jcode = function (self) {
    self.eslah = function (r, dty, dtm, dtd) {
        ...
        var arr = res.split(":");
        var specificUsers = [1908, 5237];
        /*commented for coronavirus  */
        if (specificUsers.indexOf(Main.UserInfo.id) >= 0) {
            if (parseInt(arr[0]) > 15) {
                Utils.showMessage('شما به دليل استفاده بيش از 15 فرم در ماه مجاز به اين عمليات نمي باشيد');
                return;
            }
        } else if (parseInt(arr[0]) > 8) {
            Utils.showMessage('شما به دليل استفاده بيش از 8 فرم در ماه مجاز به اين عمليات نمي باشيد');
            return;
        }
        ...
        if (dateDiff == 0 || (dateDiff == 1 && parseInt(nowDate[2]) <= 10)) {
            dt = dty + '/' + dtm + '/' + dtd;
