this.jcode = function(self) {
    try {
        self.LoadJS = function(time, id) {
            var codem = Main['UserInfo']['employeeID'];
            FormOnly.allFieldsContianer[5].setData(codem);
            $jq('.FarhangiCodeM').text(codem);
        ...
        self.DoreZamaniJS = function(fw) {
            ...
            var mm = '';
            var yy = '';
            codem = FormOnly.allFieldsContianer[5].getData();
            mm = FormOnly.allFieldsContianer[2].getData();
            parseInt(mm++);
            if (mm.toString().length == 1) mm = '0' + parseInt(mm);
            yy = FormOnly.allFieldsContianer[3].getData();
            yy = parseInt(yy) + 1398;
            var res = Utils.fastAjax('WorkFlowAjaxFunc', 'MehrNewsTimexList', { mm: mm, yy: yy, codem: codem, status: 'DoreZamani' });
        ...
        self.BazeZamaniJS = function(fw) {
            ...
            if (azt == "") {
                ...
                if (azt > tat) {
                    ...
                    var res = Utils.fastAjax('WorkFlowAjaxFunc', 'MehrNewsTimexList', { aztd: aztd, aztm: aztm, azty: azty, tatd: tatd, tatm: tatm, taty: taty, codem: codem, status: 'BazeZamani' });
        ...
        self.CreateForm = function(FormWFID, FormType, FormDate, FormCodeM) {
            ...
            Utils.fastAjax('NForms', 'setData', { data: '{"' + FormDate_ID + '":"' + FormDate + '", "' + FormCodeM_ID + '":"' + FormCodeM + '"}', docid: res.docID, fieldid: res.formID, referid: res.referID, ttype: 'form' });
