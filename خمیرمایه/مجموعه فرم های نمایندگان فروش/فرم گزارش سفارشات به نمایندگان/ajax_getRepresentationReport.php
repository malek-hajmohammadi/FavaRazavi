...
class MainAjax
{
    ...
    private function getPageNumber()
    {
        ...
        $this->searchFields->rowsEnd = $pageNumber * self::PAGE_LENGTH;
    }
    ...
    private function whereClause()
    {
        ...
        $whereClause = " where  oa_document.CreatorRoleID=$RID AND ";
        ...
        $whereClause = $whereClause . $formNumberSt . $firstDate . $endDate .
            $cableState . $productNameSt;
        $whereClause .= "1 ";
    ...
    private function getCableState()
    {
        ...
        if ($cableState == "-1" ||$cableState == "2") {
            $cableStateString = "Field_5 !='namayandeh' and "; //پیش نویس ها رو نمی خواهم نشون بدم//
...
INNER JOIN oa_document on (oa_document.rowid = dm.docId) " . $stForHavingProductName;
        ...
        $sqlLimit = "ORDER BY dm.RowID DESC limit {$this->searchFields->rowsStart}," . $this->searchFields->rowsEnd;
        /*
        $PDOParams = array(
...
INNER JOIN oa_document on (oa_document.rowid = dm.docid)" . $stForHavingProductName;
        $sqlWhereClause = $this->whereClause();
        $sqlLimit = "ORDER BY dm.RowID limit {$this->searchFields->rowsStart}," . $this->searchFields->rowsEnd;
...
*/
/*اینها رو کامنت کردم چون نمی خواهم فرم های پیش نویس رو به دلیل مشکلی که در باز کردن فایلهای پیش نویس در خمیرمایه داریم ، نشون بدم*/
        return $referId;
...
<div style="text-align: center;padding: 2px;">
                ...
                <table width="100%" class="f-table accessTable" cellpadding="0" cellspacing="1" >     
                    <tbody>
                        <tr>
                            ...
                            <th width="10%" style="padding: 2px; ">لینک به فرم</th>
    ...
    private function body($dataFromDatabase)
    {
        ...
        foreach ($dataFromDatabase as $value) {
            ...
            $createDate = $value[7];
            $referID=$this->getReferIdForLink($docId);
            $link='<div class="f-button-2" onmousedown="window.codeSet.linkToForm('.$referID.',\''.$this->searchFields->linkMode.'\')">لینک</div>';
            ...
            $bodyPart .= '<tr id="accessRow_' . ($radif) . '" >
                        ...
                        <td style="padding: 2px;border: 1px solid #ccc;" >' . $link . '</td>
